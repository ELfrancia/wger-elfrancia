// Muscle SVG path coordinates from react-body-highlighter
window.anteriorData = [
    {
        muscle: 'chest',
        svgPoints: [
...
// Helper to draw body outlines
function generateSvgBody(svgId, dataList, isFront) {
    const svg = document.getElementById(svgId);
    if (!svg) return;
    svg.innerHTML = ''; // Clear contents

    const ns = 'http://www.w3.org/2000/svg';

    // Draw hands and feet to complete the body shape
    if (isFront) {
        // Left Hand
        const lHand = document.createElementNS(ns, 'polygon');
        lHand.setAttribute('points', '0,100 2,106 5,108 8,107 4,97');
        lHand.setAttribute('fill', '#31333E');
        lHand.setAttribute('stroke', '#0C0D0F');
        lHand.setAttribute('stroke-width', '0.6');
        svg.appendChild(lHand);

        // Right Hand
        const rHand = document.createElementNS(ns, 'polygon');
        rHand.setAttribute('points', '100,100 98,106 95,108 92,107 96,97');
        rHand.setAttribute('fill', '#31333E');
        rHand.setAttribute('stroke', '#0C0D0F');
        rHand.setAttribute('stroke-width', '0.6');
        svg.appendChild(rHand);

        // Left Foot
        const lFoot = document.createElementNS(ns, 'polygon');
        lFoot.setAttribute('points', '20,195 21,200 25,200 27,194');
        lFoot.setAttribute('fill', '#31333E');
        lFoot.setAttribute('stroke', '#0C0D0F');
        lFoot.setAttribute('stroke-width', '0.6');
        svg.appendChild(lFoot);

        // Right Foot
        const rFoot = document.createElementNS(ns, 'polygon');
        rFoot.setAttribute('points', '80,195 79,200 75,200 73,194');
        rFoot.setAttribute('fill', '#31333E');
        rFoot.setAttribute('stroke', '#0C0D0F');
        rFoot.setAttribute('stroke-width', '0.6');
        svg.appendChild(rFoot);
    } else {
        // Back Hands
        const lHand = document.createElementNS(ns, 'polygon');
        lHand.setAttribute('points', '0,106 2,112 5,114 8,113 4,103');
        lHand.setAttribute('fill', '#31333E');
        lHand.setAttribute('stroke', '#0C0D0F');
        lHand.setAttribute('stroke-width', '0.6');
        svg.appendChild(lHand);
        ...
        const rHand = document.createElementNS(ns, 'polygon');
        rHand.setAttribute('points', '100,106 98,112 95,114 92,113 96,103');
        rHand.setAttribute('fill', '#31333E');
        rHand.setAttribute('stroke', '#0C0D0F');
        rHand.setAttribute('stroke-width', '0.6');
        svg.appendChild(rHand);
    ...
    // Loop and render muscle polygons
    dataList.forEach(item => {
        item.svgPoints.forEach(points => {
            const poly = document.createElementNS(ns, 'polygon');
            poly.setAttribute('points', points);
            poly.setAttribute('class', 'muscle-polygon transition-all duration-300');
            poly.setAttribute('data-muscle', item.muscle);
            poly.setAttribute('fill', '#31333E');
            poly.setAttribute('stroke', '#0C0D0F');
            poly.setAttribute('stroke-width', '0.6');
            poly.style.cursor = 'pointer';
            
            // Subtle hover effect
            poly.addEventListener('mouseenter', () => {
                if (poly.getAttribute('fill') === '#31333E') {
                    poly.setAttribute('fill', '#414452');
                }
            });
            poly.addEventListener('mouseleave', () => {
                if (poly.getAttribute('fill') === '#414452') {
                    poly.setAttribute('fill', '#31333E');
                }
            });
...
function normalizeMuscleName(name) {
    ...
    if (n.includes('pectoral') || n.includes('chest') || n.includes('petto') || n.includes('grand pettorale') || n.includes('brachiale')) return 'chest';
    if (n.includes('abdominis') || n.includes('abs') || n.includes('addome') || n.includes('abdominals') || n.includes('retto dell\'addome')) return 'abs';
    if (n.includes('glute') || n.includes('gluteus') || n.includes('gluteal') || n.includes('grande gluteo')) return 'gluteal';
    if (n.includes('calves') || n.includes('calf') || n.includes('gastrocnemius') || n.includes('polpacci') || n.includes('gemelli')) return 'calves';
    if (n.includes('soleus') || n.includes('soleo')) return 'soleus'; // will highlight both left-soleus and right-soleus
    if (n.includes('trapezius') || n.includes('traps') || n.includes('trapezio')) return 'trapezius';
    if (n.includes('latissimus') || n.includes('lats') || n.includes('dorsali') || n.includes('grand dorsale') || n.includes('upper back') || n.includes('upper-back')) return 'upper-back';
    if (n.includes('lower back') || n.includes('lower-back') || n.includes('erector') || n.includes('lombari') || n.includes('erettori spinali')) return 'lower-back';
    if (n.includes('forearm') || n.includes('brachioradialis') || n.includes('avambracci') || n.includes('flessori') || n.includes('estensori del polso')) return 'forearm';
    if (n.includes('oblique') || n.includes('obliqui')) return 'obliques';
    if (n.includes('neck') || n.includes('collo')) return 'neck';
    if (n.includes('deltoid') || n.includes('spalle') || n.includes('shoulder') || n.includes('deltoide anteriore') || n.includes('deltoide posteriore')) {
        return 'deltoids';
    }
    if (n.includes('abductor') || n.includes('abduttori')) return 'abductors';
    if (n.includes('adductor') || n.includes('adduttori')) return 'adductor';
    if (n.includes('hamstring') || n.includes('femorali') || n.includes('bicipite femorale') || n.includes('ischiocrurali')) return 'hamstring';
    if (n.includes('quadriceps') || n.includes('quadricipiti') || n.includes('quadricipite femorale')) return 'quadriceps';
    if (n.includes('knee') || n.includes('ginocchia')) return 'knees';
    return n;
}

function isMuscleMatch(polygonMuscle, normalized) {
    if (polygonMuscle === normalized) return true;
    if (normalized === 'soleus' && (polygonMuscle === 'left-soleus' || polygonMuscle === 'right-soleus')) return true;
    if (normalized === 'deltoids' && (polygonMuscle === 'front-deltoids' || polygonMuscle === 'back-deltoids')) return true;
    return false;
}

window.highlightMusclesInModal = function(primaryMuscles, secondaryMuscles) {
    // Generate fresh bodies first to make sure polygons exist
    generateSvgBody('front-body-svg', window.anteriorData, true);
    generateSvgBody('back-body-svg', window.posteriorData, false);

    const primaryNormalized = primaryMuscles.map(m => {
        const name = typeof m === 'string' ? m : (m.name_en || m.name);
        return normalizeMuscleName(name);
    });
    const secondaryNormalized = secondaryMuscles.map(m => {
        const name = typeof m === 'string' ? m : (m.name_en || m.name);
        return normalizeMuscleName(name);
    });

    const polygons = document.querySelectorAll('.muscle-polygon');
    polygons.forEach(poly => {
        const polyMuscle = poly.getAttribute('data-muscle');
        
        let isPrimary = false;
        for (let pm of primaryNormalized) {
            if (isMuscleMatch(polyMuscle, pm)) {
                isPrimary = true;
                break;
            }
        }
        ...
        for (let sm of secondaryNormalized) {
            if (isMuscleMatch(polyMuscle, sm)) {
                isSecondary = true;
                break;
            }
        }
